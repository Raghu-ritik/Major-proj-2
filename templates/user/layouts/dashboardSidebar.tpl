{literal}
    <style>
    .nav-label{
        margin-top: 25px;
        font-weight: bold;
    }
    @media only screen and (max-width: 600px) {
        .left-sidebar {
            display: none;
        }
    }
    </style>
    {/literal}
    
           <div class="left-sidebar">
               
               <div class="scroll-sidebar">
                   
                   <nav class="sidebar-nav">
                       <ul id="sidebarnav">
                           <li class="nav-devider"></li>
                           <li class="nav-label"><h2>Home</h2></li>
                           <li><a href="" aria-expanded="false"><i class="fa fa-tachometer"></i>Dashboard</a> </li>
                           <li> <a class="has-arrow" href="track_order.php" aria-expanded="false"><i class="fa fa-files-o"></i><span class="hide-menu">Track order</span></a>                </li>
                           <li><a href="contact.php" aria-expanded="false"><i class="fa fa-commenting-o"></i><span class="hide-menu">Contact us</span></a></li>
                           <li><a href="knowMore.php" aria-expanded="false"><i class="fa fa-info-circle"></i><span class="hide-menu">Know More</span></a></li>
                           <li><a class="btn-book-a-table" href="cart.php"> <i class="fa fa-cart-plus" aria-hidden="true"></i>  Cart</a></li>
                           <li><a href="logout.php" aria-expanded="false"><i class="fa fa-sign-out"></i><span class="hide-menu">logout</span></a></li>              
                        </ul>    
                   </nav>
                   
               </div>
               
           </div>
           